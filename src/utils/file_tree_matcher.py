...
class FileTreeMatcher:
    def __init__(self, root_dir: str, white_extension: list, black_song: list) -> None:
    ...
    def build_file_tree_from_m3u(self, m3u_directory: str) -> None:
        # root_dir存储音乐库的根目录
    ...
    def get_path_mapping(self, old_file_tree: dict) -> dict:
        path_mapping = {}
        self._compare_file_trees(self.file_tree, old_file_tree, path_mapping, "")
        return path_mapping

    def _compare_file_trees(self, new_tree: dict, old_tree: dict, path_mapping: dict, new_path: str) -> None:
            ...
            if isinstance(new_node, dict):  # 如果是目录节点
                ...
                if old_path is not None:  # 如果在旧文件树中存在相同文件
                    ...
                    if old_path != new_path:
                        ...
                        path_mapping[os.path.join(old_path, name + extension)] = os.path.join(
                            new_path, name + extension
                        )

    def _find_file_in_tree(self, file_name: str, tree: dict, old_path: str) -> str:
