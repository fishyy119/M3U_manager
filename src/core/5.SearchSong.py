...
def find_file_in_m3u(m3u_directory: str, search_result: dict, target: list) -> int:
    count = 0
...
def find_file_in_tree(
    node: dict, search_result: dict, path: str, count: list, target: list  # 初次调用时定义为[0]以进行引用传递
) -> int:
